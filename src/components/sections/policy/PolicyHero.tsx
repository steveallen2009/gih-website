...
export default function PolicyHero({ title, lastUpdated }: PolicyHeroProps) {
  const { theme } = useTheme()
  const isDark = theme === 'dark'

  return (
    <section
      className="relative min-h-[50vh] flex items-end overflow-hidden transition-colors duration-300"
      style={{
        background: isDark
          ? `linear-gradient(
              180deg,
    ...
    >
      ...
      {/* Ambient glow orb */}
      <div
        className="absolute -top-48 -left-36 w-[620px] h-[620px] rounded-full pointer-events-none"
        style={{
          background: isDark
            ? 'rgba(212,168,67,0.035)'
            : 'rgba(212,168,67,0.055)',
          filter: 'blur(150px)',
        }}
      />
      ...
      <div className="wrap relative z-10">
        ...
        <motion.p
          className="text-base mt-5 max-w-lg transition-colors duration-300"
          style={{
            color: isDark
        ...
        >
          Last updated: <span className="font-mono text-sm">{lastUpdated}</span>
        </motion.p>
      </div>
